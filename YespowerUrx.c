...
int scanhash_urx_yespower(int thr_id, uint32_t *pdata,
    ...
    if (!data_u8 || !hash_u32) {
        ...
        free(data_u8); // Free any allocated memory
        free(hash_u32); // Free any allocated memory
        return 0; // Early return in case of failure
    }

    // Prepare initial data for hashing
    for (int i = 0; i < 19; i++) {
        be32enc(&data_u8[i], pdata[i]); // Ensure endianess is handled correctly
    }
    ...
    while (n < max_nonce) {
        for (int j = 0; j < UNROLL_FACTOR; j++) {
            // Check if we exceed max_nonce
            if (n + j * NONCE_BATCH_SIZE >= max_nonce) {
                break; // Exit if exceeding max_nonce
            }

            uint32x4_t nonce_vec = vdupq_n_u32(n + j * NONCE_BATCH_SIZE); // Calculate nonce vector
            ...
            if (yespower_tls(data_u8, 80, &params, (yespower_binary_t *)hash_u32)) {
                ...
                free(data_u8); // Clean up memory on failure
                free(hash_u32);
                abort();
            }
